...
<template>
  <DashboardProvider>
    ...
    <div class="grid grid-cols-4 w-full mt-4 gap-2">
      <Card
        v-for="cosmetic in cosmeticsData"
        v-if="!isLoading"
        class="relative"
      >
        <Dialog>
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              ...
              <DialogTrigger>
                <DropdownMenuItem
                  @click="openDialogType = 'delete'"
                  class="text-red-600 focus:bg-red-500 focus:text-white"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
          ...
          <DialogContent v-if="openDialogType == 'give'">
            <DialogHeader>
              <DialogTitle
                >Give {{ getNameFromType(cosmetic.type) }}
                {{ cosmetic.displayName }}</DialogTitle
              >
              <DialogDescription
                >Enter the name of the player you want to give this cosmetic
                to</DialogDescription
              >
            </DialogHeader>

            <div class="py-4 pt-2">
              <Label for="giveToPlayerName" class="text-sm font-medium"
                >Player name</Label
              >
              <Input
                id="giveToPlayerName"
                type="text"
                placeholder="Gezelligheid_"
              />
            </div>
        ...
        <CardHeader>
          ...
          <CardDescription>{{
            cleanedDescription(cosmetic.description)
          }}</CardDescription>
        </CardHeader>
        <CardContent>
